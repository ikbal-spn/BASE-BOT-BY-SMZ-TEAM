...
┌|  *GROUP MENU* 👥
│ฅ ${prefix}antilink on/off - Aktifkan/matikan antilink
│ฅ ${prefix}welcome on/off - Aktifkan/matikan welcome
└— ◦
...
│ฅ > (eval)
│ฅ $ (exec)
│ฅ ${prefix}newgc (buat gc new)
│ฅ ${prefix}join (link)
│ฅ ${prefix}leave (grup)
│ฅ ${prefix}bcgroup - Broadcast ke grup
│ฅ ${prefix}bcmember - Broadcast ke member
│ฅ ${prefix}bcadmin - Broadcast ke admin
...
Scan QRIS di bawah untuk melakukan deposit saldo:`;
        ...
        await sock.sendMessage(from, { 
          ...
          caption: depositMessage + '\n\n' + settings.storeSettings.paymentInstructions.join('\n')
        }, { quoted: msg });
      } catch (error) {
...
Mohon cek bukti pembayaran yang akan dikirimkan. 📝` 
    ...
    case 'bc':
      ...
      // Format: bc [text] [delay] [target]
      if (args.length < 3) {
        await sock.sendMessage(from, { 
          text: `📝 Penggunaan: ${prefix}bc [text] [delay] [link grup]` 
        }, { quoted: msg });
        break;
      }
      ...
      try {
        // Extract parameters
        const delayIndex = args.findIndex(arg => !isNaN(arg) && Number(arg) > 0);
        
        if (delayIndex === -1) {
          await sock.sendMessage(from, { 
            text: '❌ Format tidak valid. Delay harus berupa angka positif.' 
          }, { quoted: msg });
          break;
        }
        
        const bcText = args.slice(0, delayIndex).join(' ');
        const delaySeconds = parseInt(args[delayIndex]);
        const targetLink = args[delayIndex + 1];
        
        if (!targetLink || !targetLink.includes('https')) {
          await sock.sendMessage(from, { 
        ...
        try {
          ...
          await sock.sendMessage(joinResult, { 
            text: `📢 *BROADCAST MESSAGE*\n\n${bcText}\n\n_Pesan ini dikirim oleh owner bot._` 
          });
          
          await sock.sendMessage(from, { 
            text: `✅ Broadcast berhasil dikirim ke grup target dengan delay ${delaySeconds} detik!` 
          }, { quoted: msg });
    ...
    case 'antilink':
      ...
      try {
        ...
        // Get or register group
        let groupData = groupFunctions.getGroup(from);
        
        if (!groupData) {
          groupFunctions.register(from, groupMetadata.subject);
          groupData = groupFunctions.getGroup(from);
        }
        
        // Update antilink setting
        groupFunctions.updateGroup(from, {
          ...groupData,
          antilink: option === 'on'
        });
        
        await sock.sendMessage(from, { 
          text: `✅ Fitur anti-link berhasil ${option === 'on' ? 'diaktifkan' : 'dinonaktifkan'}!` 
        }, { quoted: msg });
